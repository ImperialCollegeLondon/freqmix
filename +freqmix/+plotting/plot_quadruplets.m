...
plot_number_quadruplets_bands_graph(mixing,mixing_combinations,config);
plot_tstat_bands_graph(mixing, mixing_combinations, config);

% plot channel relationships
plot_number_quadruplets_channels_graph(mixing,mixing_combinations,config);
...
function plot_number_quadruplets_bands_graph(mixing, mixing_combinations, config)
...
% generate graph
g = generate_quadruplet_graph(banded_quadruplets,banded_tested_quadruplets,band_ids);    
...
hcb=colorbar; ylabel(hcb,'Normalised # quadruplets'); caxis([0,max(n_quads_band,[],'all')])
...
p.EdgeAlpha = 1;
if config.title
    title(['Frequency bands: # quadruplets normalised']); 
end
plot_params(gca);
cbarrow(max_edge);
...
if config.saveplot
    savefig([config.folder,'quadruplets_nquadruplets_freqbands_network.fig'])
    close all;
end
...
function plot_number_quadruplets_channels_graph(mixing,mixing_combinations,config)
...
for ch = 1:length(channels)
    ...
    n_quads_ch(ch) = sum(sum(channel_presence))/n_tested_quads_ch(ch);
end
...
g = generate_graph(channeled_quadruplets,channeled_tested_quadruplets,channels);    
